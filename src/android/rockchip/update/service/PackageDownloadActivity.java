...
public class PackageDownloadActivity extends Activity {
    ...
    private int mAutoStartDelay = 5000;
        ...
        mBtnControl.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                if (mState == STATE_READY || mState == STATE_ERROR) {
                    mTask = new FileDownloadTask(mHttpClient, mUri, "/data/media/0", mFileName, 3);
    ...
    private class ProgressHandler extends Handler {
        public void handleMessage(Message msg) {
            ...
            case FileDownloadTask.PROGRESS_DOWNLOAD_COMPLETE:
                ...
                intent.putExtra(RKUpdateService.EXTRA_IMAGE_PATH, "/data/media/0/" + mFileName);
            ...
            case FileDownloadTask.PROGRESS_STOP_COMPLETE:
                ...
                // Automatic perform retry after mAutoStartDelay seconds
                mAutoHandler  = new Handler();
                mAutoRunnable = new Runnable() {
                    @Override
                    public void run() {
                        long elapsed = (System.nanoTime() - mAutoStartTime) / 1000000;
                        if (elapsed > mAutoStartDelay && mState == STATE_ERROR) {
                            mBtnControl.performClick();
                        } else if (mState == STATE_ERROR) {
                            int secRemaining = (int) Math.round((mAutoStartDelay-elapsed)/1000);
                            mBtnControl.setText(getString(R.string.retry) + " (" + Integer.toString(secRemaining) + ")");
